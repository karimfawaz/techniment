...
def create_and_train_model():
    ...
    # if abs(y_new - y_test_pred.mean()) < 1.96 * std:
    #     print(y_new)

    # # Get the probability estimates for the test data
    # y_test_prob = svr.predict_proba(X_test)

    # # Iterate through each test instance and check if it is 95% confident
    # for i in range(len(y_test)):
    #     if np.max(y_test_prob[i]) >= 0.95:
    #         # Make a prediction using the SVR model
    #         y_test_pred = svr.predict([X_test[i]])
    #         print("Prediction:", y_test_pred)
    #     else:
    #         print("Not confident enough to make a prediction")

    # save model
    pickle.dump(svr, open("ta_svm.pkl", "wb"))


create_and_train_model()
# Load the saved model from a file
# with open('ta_svm.pkl', 'rb') as file:
